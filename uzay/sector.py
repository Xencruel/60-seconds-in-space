...
class SpaceShip(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.speed_y = 100  # Y eksenindeki hareket hızı
        self.speed_x = 100  # X eksenindeki hareket hızı
...
def reset_game():
    ...
    t = threading.Thread(target=timer, args={time_limit, })
    t.start()
    for _ in range(8):
        alien = Alien()
...
while not game_exit:
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_q):
            game_exit = True
        elif game_started == False and event.type == pygame.KEYDOWN:
            print("game not started and keydown detected")
            print(f"Event key: {event.key} | expected : {pygame.K_SPACE}")
    ...
    if game_started:
        ...
        time_left = font.render(f"Süre: {tl}", True, WHITE)
        score_text = font.render("Skor: {}".format(player.score), True, WHITE)
        screen.blit(lives_text, (10, 10))
        screen.blit(time_left, (10, 35))
